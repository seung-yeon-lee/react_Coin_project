...
const PAGE_SIZE = 10;
const { collection, create, reset } = createActions('transactions'); //커링패턴 참조

export const resetTransactionList = reset;

export function requestTransactionList(params, _page = 1) {
  //action 묶음함수를 사용하여 request...액션함수 변경 code (액션 모듈화 작업)
  const meta = {
    pageNumber: _page,
    pageSize: PAGE_SIZE,
    notification: {
...
// 2개의 action 모두 promise, type만 사라지고 묶음 함수를 반환하며 호출
